...
export const ProjectService = {
  ...
  async board(projectId: string, user: UserContext) {
    ...
    const mapTask = (t: any) => ({
      ...
      blockedByDependencies: t.dependencies.some(
  ...
  async create(data: any, user: UserContext) {
    const access = getAccessLevel(user);
    if (access !== 'pm')
      throw new AppError(403, 'Only PM can create', 'FORBIDDEN');
  ...
  async softDelete(projectId: string, user: UserContext) {
    const access = getAccessLevel(user);
    if (access !== 'pm')
      throw new AppError(403, 'Only PM can delete', 'FORBIDDEN');
